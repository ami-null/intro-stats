...
\title{Common Probabiilty Distributions}
...
\begin{document}
	...
	\begin{frame}{Bernoulli Distribution: Exercises}
		\begin{enumerate}
			\item If \(X \sim \mathrm{Bernoulli}(0.3)\), find \(\E(X)\) and \(\V(X)\).
			\item If \(X \sim \mathrm{Bernoulli}(p)\) and it is known that \(\V(X) = 0.21\), find the two possible values of \(p\).
		\end{enumerate}
	\end{frame}
	...
	\begin{frame}{Binomial Distribution: Exercises}
		\begin{enumerate}
			\item A fair coin is tossed \(6\) times. Find the probability of getting exactly \(4\) heads.
			\item In a batch of \(10\) items, the probability that any item is defective is \(0.1\). Find the probability that at most \(2\) items are defective.
			\item If \(X \sim \mathrm{binom}(20, 0.4)\), find \(\E(X)\), \(\V(X)\), and \(P(X=8)\).
		\end{enumerate}
	\end{frame}
	...
	\begin{frame}{Poisson Distribution: Exercises}
		\begin{enumerate}
			\item The number of typing errors on a page follows a Poisson distribution with mean \(3\). Find the probability that a randomly chosen page has no errors, and the probability that it has more than \(2\) errors.
			\item Calls arrive at a call center at a rate of \(5\) per minute. Find the probability of exactly \(2\) calls in the next minute, and the probability of at least one call in the next \(30\) seconds.
			\item If \(X \sim \mathrm{Poisson}(4)\), find \(P(X \leqslant 1)\) and \(\operatorname{SD}(X)\).
		\end{enumerate}
	\end{frame}
	...
	\begin{frame}{Uniform Distribution: Example 1}
		The waiting time (in minutes) for train is uniform($10, 50$). Find the following: \label{train-example}
		\begin{enumerate}
	...
	\begin{frame}{Uniform Distribution: Example 1 Solution}
		Here, wait time, \(X \sim \mathrm{uniform}(10, 50)\)
		\begin{enumerate}
			\item Probability of waiting at least 20 minutes:
			\begin{align*}
				P(X>20) = P(20 \leqslant X \leqslant 50) &= \int_{20}^{50} \frac{1}{50-10} \, dx\\
	...
	\begin{frame}{Uniform Distribution: Example 2}
		Find the CDF of \(X \sim \mathrm{uniform}(a, b)\). Then find the CDF the train station waiting example in page \ref{train-example}, i.e., when \(a = 10\) and \(b = 50\). Using this CDF, find the following probabilites:
		\begin{enumerate}
			\item Probability that you have to wait less than \(25\) minutes
			\item Probablity that you have to wait more than 25 minutes
			\item Probability that the wait time is between 26 and 36 minutes
			\item Probability that the wait time is less than 32 minutes or more than 42 minutes
		\end{enumerate}
	\end{frame}


	\begin{frame}{Uniform Distribution: Example 2 Solution}
		The CDF is given by:
		\begin{align*}
			F(X) = P(X < x) &= \int_{a}^{x} f(x) \, dx \\
			&= \int_{a}^{x} \frac{1}{b-a} \, dx \\
			&= \Big[\frac{x}{b-a}\Big]_a^x \\
			&= \frac{x-a}{b-a}
		\end{align*}
	\end{frame}


	\begin{frame}{Uniform Distribution: Example 2 Solution (cont.)}
		...
		\begin{enumerate}
			\item \(P(X < 25) = F(25) = \frac{25-10}{40} = 15/40\)
			\item \(P(X > 25) = 1 - F(25)\)
			\item \(P(26 < X < 36) = F(36) - F(26)\)
			\item \(P(X < 32 \cup X > 42) = P(X < 32) + P(X > 42) = F(32) + 1 - F(42)\)
		\end{enumerate}
	\end{frame}

	
	\begin{frame}{Uniform Distribution: Example 3}
		Derive the expected value of the uniform distribution:

		\begin{align*}
			E(X) &= \int_{a}^{b} x \cdot \frac{1}{b-a} \mathrm{d}x \\
	...
	\begin{frame}{Uniform Distribution: Exercises}
		\begin{enumerate}
			\item If \(X \sim \mathrm{uniform}(0, 10)\), find \(P(2 < X < 8)\), \(\E(X)\), and \(\V(X)\).
			\item A bus arrives at a stop at a time uniformly distributed between \(0\) and \(15\) minutes past the hour. Find the probability that you have to wait more than \(10\) minutes, and find the CDF of the waiting time.
		\end{enumerate}
	\end{frame}
	...
	\begin{frame}{Exponential Distribution: Example 1}
		...
		\[
			P(X>2) = \int_{2}^{\infty} 2 e^{-2x} dx = \big[-e^{-2x}\big]_2^{\infty} = \big[-0 + e^{2 \times 2}\big] = e^{-4} = 0.0183.
		\]
	\end{frame}


	\begin{frame}{Exponential Distribution: Exercises}
		\begin{enumerate}
			\item Find the CDF of the exponential distribution.
			\item The lifetime of an electronic component is exponentially distributed with mean \(5\) years. Find the probability that the component lasts more than \(8\) years.
			% \item Show that the exponential distribution is memoryless, i.e., \(P(X > s+t \mid X > s) = P(X>t)\) for \(X \sim \mathrm{exponential}(\lambda)\).
		\end{enumerate}
	\end{frame}
	...
	\begin{frame}{Normal Distribution}
		...
		\[
			...
			\mathrm{exp}\Big[-\frac{(x-\mu)^2}{2\sigma^2}\Big] \, ;
			\quad -\infty < x < \infty
		\]
	...
	\begin{frame}{Symmetry of the Standard Normal Distribution}
	...
	\begin{frame}{Common Probability Intervals}
	...
	\begin{frame}{Example: Finding Probability of Normal Random Variable}
		Let a random variable \(X \sim \N(10, 25)\). Find \(P(X<4) \text{ and } P(X>16)\):

		\[
			P(X < 4) = P\left(\frac{X-10}{\sqrt{25}} < \frac{4-10}{\sqrt{25}}\right) = P(Z < -1.2) = \Phi(-1.2) = 0.115
		\]
		\begin{align*}
			P(X > 16) = P\left(\frac{X-10}{\sqrt{25}} > \frac{16-10}{\sqrt{25}}\right) = P(Z > 1.2) &= 1 - P(Z < 1.2)\\
			&= 1 - \Phi(1.2)\\ &= 1 - 0.885\\ &= 0.115
		\end{align*}
	...
	\begin{frame}{Normal Distribution: Exercises}
		\begin{enumerate}
			\item If \(X \sim \mathrm{N}(50, 64)\), find \(P(X<40)\), \(P(X>66)\), and \(P(45<X<60)\).
			\item Test scores are normally distributed with mean \(75\) and standard deviation \(8\). Find the score below which \(90\%\) of students score.
			% \item Using the symmetry of the standard normal distribution, show that \(P(Z<-1.5) = P(Z>1.5)\), and find this probability using the Z-table.
		\end{enumerate}
	\end{frame}

	
	% \section{Approximating Probability of One Distribution with Another}

	% \begin{frame}{Approximating One Distribution with Another}
	% 	In many practical situations, a complicated distribution can be approximated by a simpler one.

	% 	\begin{itemize}
	% 		\item Exact probability calculations may be difficult for large parameters
	% 		\item Approximation simplifies computation
	% 		\item Several well-known relationships exist between common distributions
	% 	\end{itemize}

	% 	In this section we discuss:
	% 	\begin{itemize}
	% 		\item Binomial approximation by Poisson
	% 		\item Binomial approximation by Normal
	% 		\item Poisson approximation by Normal
	% 		\item Relationship with Exponential distribution
	% 	\end{itemize}
	% \end{frame}


	% \begin{frame}{Binomial Approximation by Poisson}
	...
	% 	\end{itemize}
	% 	then the binomial distribution can be approximated by a Poisson distribution:
	% 	\[
	% 		X \approx \text{Poisson}(\lambda), \quad \lambda = np
	% 	\]
	...
	% \begin{frame}{Binomial Approximation by Normal}
	% 	Suppose \(X \sim \text{Binomial}(n,p)\).

	% 	When \(n\) is large and \(p\) is not extremely close to 0 or 1, the binomial distribution can be approximated by a normal distribution.
	% 	\[
	...
	% 	A continuity correction is usually applied when using this approximation.
	% \end{frame}


	% \begin{frame}{Poisson Approximation by Normal}
	% 	Suppose \(X \sim \text{Poisson}(\lambda)\).

	% 	When \(\lambda\) is large, the Poisson distribution becomes approximately normal.
	% 	\[
	...
	% 	As with the binomial case, a continuity correction is recommended.
	% \end{frame}
