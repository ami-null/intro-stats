...
\begin{document}
	...
	\begin{frame}{Example: Finding Probability from CDF of a Continuous RV}
		Suppose the CDF of a random variable \(X\) is
		\[
			F(x) = x^2 \quad \text{for } 0 \leqslant x \leqslant 1
		\]

		Find \(P(0.2 < X < 0.6)\) and \(P(0.2 \leqslant X \leqslant 0.6)\).

		\vspace{0.5em}

		Using the CDF rule:
		\begin{align*}
			P(0.2 < X < 0.6) = F(0.6) - F(0.2) &= (0.6)^2 - (0.2)^2 \\
			&= 0.36 - 0.04 = 0.32.
		\end{align*}

		Since \(X\) is continuous, \(P(X=a)=0\).

		Therefore, \(P(0.2 \leqslant X \leqslant 0.6) = 0.32\).
	\end{frame}
	...
	% \begin{frame}{Properties of Expectation and Variance: Exercise}
