...
\begin{document}
	...
	\begin{frame}{Scope of Statistics with Examples}
		\begin{itemize}
			\item Medicine: evaluating the effectiveness of a new drug using clinical trial data
			\item Epedemiology: assessing the rate at which a disease spreads
			\item Education: analyzing exam results to compare teaching methods
			\item Economics: studying inflation unemployment and income trends
			\item Business: forecasting sales and analyzing customer behavior
			\item Engineering: monitoring process quality and detecting defects
			\item Social sciences: understanding public opinion through surveys
		\end{itemize}
	\end{frame}


	\begin{frame}{Statistics in Engineering}
		\begin{itemize}
			\item Monitoring and controlling manufacturing process quality using control charts
			\item Analyzing failure rates and predicting the reliability and lifespan of components
			\item Designing experiments to optimize product performance and material properties
			\item Detecting defects and reducing variability in production processes
			\item Modeling and simulating systems to evaluate engineering designs
		\end{itemize}		
	\end{frame}


	\begin{frame}{Statistics in Biotechnology}
		\begin{itemize}
		\item Analyzing gene expression data
		\item Comparing growth rates of genetically modified and non modified organisms
		\item Estimating mutation rates from DNA sequencing data
		\item Assessing the effectiveness of gene therapy treatments
		\item Designing and conducting experiments for drug and vaccine development
	\end{itemize}
	\end{frame}
	...
	\begin{frame}{Census and Survey}
		...
		\item Census:
			\begin{itemize}
				\item Data are collected from every unit in the population
				\item Provides complete information with no sampling error
				\item Usually time consuming and expensive
				\item Suitable for small or well defined populations
			\end{itemize}
			\item Survey:
			\begin{itemize}
				\item Data are collected from a sample of the population
				\item Results are subject to sampling variability
				\item Faster and more cost effective
				\item Suitable for large or infinite populations
				\item Allows estimation and inference about the population
			\end{itemize}
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Example: Interval Scale Variables}
		\begin{itemize}
			\item \textbf{Temperature (°C or °F):} The difference between 20°C and 30°C is the same as between 30°C and 40°C --- intervals are equal and meaningful. However, 0°C does not mean ``no temperature,'' so ratios are meaningless: 40°C is \textit{not} twice as hot as 20°C
			\item \textbf{IQ Score:} The difference between scores of 90 and 100 is the same as between 110 and 120. However, a score of 0 does not mean the complete absence of intelligence, so we cannot say someone with IQ 140 is ``twice as intelligent'' as someone with IQ 70
			\item \textbf{Calendar Year:} The gap between year 1900 and 1950 equals that between 1950 and 2000. However, year 0 is an arbitrary reference point, not a true absence of time, so saying year 2000 is ``twice as recent'' as year 1000 is meaningless
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Example: Ratio Scale Variables}
		\begin{itemize}
			\item \textbf{Height:} A height of 0\,cm means the complete absence of height. Equal intervals exist between measurements, and ratios are meaningful --- a person who is 180\,cm is exactly twice as tall as a child who is 90\,cm
			\item \textbf{Weight:} Zero weight means no mass at all. A package weighing 10\,kg is genuinely twice as heavy as one weighing 5\,kg, making both differences and ratios interpretable
			\item \textbf{Income:} Zero income means no earnings whatsoever. Someone earning \$60{,}000 per year earns exactly three times as much as someone earning \$20{,}000, which is a meaningful and interpretable ratio
			\item \textbf{Reaction Time:} A reaction time of 0 would indicate an instantaneous response (true zero). A response time of 400\,ms is twice as long as one of 200\,ms, and this comparison is physically meaningful
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Observational Data}
		...
		\begin{itemize}
			\item Variables are observed as they naturally occur
			\item Common in surveys and observational studies
			\item Advantages:
			\begin{itemize}
				\item Easier and cheaper to collect
				\item Suitable when experiments are impractical or unethical
				\item Reflects real world conditions
			\end{itemize}
			\item Disadvantages:
			\begin{itemize}
				\item Cannot establish cause and effect relationships
				\item More vulnerable to confounding factors
			\end{itemize}
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Experimental Data}
		...
		\begin{itemize}
			\item Presence of control and treatment groups
			\item Random assignment of experimental units
			\item Advantages:
			\begin{itemize}
				\item Allows identification of cause and effect relationships
				\item Greater control over experimental conditions
				\item Reduces the impact of confounding variables
			\end{itemize}
			\item Disadvantages:
			\begin{itemize}
				\item Often time consuming and expensive
				\item May face ethical or practical limitations
				\item Artificial settings may reduce realism
			\end{itemize}
		\end{itemize}
	\end{frame}
