...
\begin{document}
    ...
    \frametitle{Markov Process}
        \begin{itemize}
            \item Consider a finite or countably infinite set of points $(t_0, t_1, \ldots, t_n, t)$, where $t_0 < t_1 < \cdots < t_n < t$ and $t, t_i \in T$ ($i = 0, 1, 2, \ldots, n$); $T$ is a parameter space of the process $\{X(t)\}$.
            \item The dependence exhibited by the process $\{X(t): t \in T\}$ is called Markovian dependence if the conditional distribution of $X(t)$ for a given value of $X(t_0), X(t_1), X(t_2), \ldots, X(t_n)$ depends only on $X(t_n)$, which is the most recent known value of the process, i.e., if
            \[
            \Pr[X(t) = x \mid X(t_n) = x_n, X(t_{n-1}) = x_{n-1}, \ldots, X(t_0) = x_0] = \Pr[X(t) = x \mid X(t_n) = x_n]
            \]
        \end{itemize}
    ...
    \begin{frame}{Types of Markov Process}
        ...
        \]
        \[
        p_{ij}^{(n)} = n\text{-step transition probability of state } j \text{ from state } i = \Pr[\text{process moves from } i \text{ to } j \text{ in } n \text{ steps}]
        \]
    \end{frame}
    ...
    \begin{frame}{Markov Process: Example 1}
        ...
        \]
        Here, $p_{11} > 0, p_{12} > 0, p_{13} > 0, p_{21} > 0, p_{22} > 0, p_{23} > 0, p_{31} = 0, p_{32} = 0, p_{33} = 1$.
    ...
    \begin{figure}
        ...
        \draw[->] (3) to[loop below] node[below] {$p_{33}$} (3);
        \draw[->] (1) -- node[above right] {$p_{12}$} (2);
        \draw[->] (2) -- node[below right] {$p_{21}$} (1);
        \draw[->] (1) -- node[left] {$p_{13}$} (3);
        \draw[->] (2) -- node[below left] {$p_{32}$} (3);
        \draw[->] (3) -- node[above] {$p_{23}$} (2);
        \end{tikzpicture}
        \end{figure}
    \end{frame}
    ...
    \begin{frame}{Example --- solution}
        ...
        \[
            \Pr[X_0 = 1, X_1 = 0, X_2 = 2] = \Pr[X_2 = 2 \mid X_1 = 0] \Pr[X_1 = 0 \mid X_0 = 1] \Pr[X_0 = 1] = p_{02} \, p_{10} \, p_1
        \]
