...
\begin{document}
    ...
    \begin{frame}{Percentile}
        \begin{itemize}
            ...
            \item The median is the 50th percentile, the first quartile is the 25th percentile, the third quartile is the 75th percentile
    ...
    \begin{frame}{Calculating Percentile}
        \begin{itemize}
                ...
                \item If $\frac{i \cdot n}{100}$ is an integer, then the average of the $\left(\frac{i \cdot n}{100}\right)^{\text{th}}$ value and the value to its right, is $p_i$
    ...
    \begin{frame}{Example: Calculating Percentile}
        Find the 90th and 20th percentiles from the test scores of 25 students: $43, 54, 56, 61, 62, 66, 68, 69, 69, 70, 71, 72, 77, 78, 79, 85, 87, 88, 89, 93, 95, 96, 98,$\\ $99, 99$.
        \vspace{1em}

        $\mathbf{90}^{\textbf{th}}$ \textbf{percentile}: $\frac{90 \times 20}{25} = 22.5$. Therefore, the $23^{\text{rd}}$ value is the $90^{\text{th}}$ percentile, which is $98$.
        \vspace{1em}

        $\mathbf{20}^{\textbf{th}}$ \textbf{percentile}: $\frac{20 \times 20}{25} = 5$. Therefore, the average of the $5^{\text{th}}$ and the $6^{\text{th}}$ values, is the desired percentile, which is $\frac{62+66}{2}=64$.
    \end{frame}
    ...
    \begin{frame}{Coefficient of Variation}
        \begin{itemize}
            ...
            \item It is useful for comparing variability between datasets with different units or different means
    ...
    \begin{frame}{Caluclating Skewnewss}
        \begin{itemize}
            \item Pearson's Coefficient of Skewnewss $= \frac{3(\text{mean}-\text{median})}{\text{Standard deviation}}$
            \item Bowley's coefficient of skewnewss $= \frac{(Q_3-Q_2)-(Q_2-Q_1)}{(Q_3-Q_1)}$
