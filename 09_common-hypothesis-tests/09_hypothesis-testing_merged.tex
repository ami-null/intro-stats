...
\begin{document}
    ...
    \begin{frame}{Choosing Sample Size via Power}
        \begin{itemize}
            ...
            \item Given a target power (typically $0.80$), an expected effect size, the variability $\sigma$, and a chosen $\alpha$, the minimum required $n$ can be calculated
            \item Rule of thumb: smaller effect size or larger $\sigma$ $\Rightarrow$ larger $n$ needed to keep power adequate
            \item Trade-off: larger $n$ costs more time and resources, but too small an $n$ risks a high $\beta$ (missing a real effect)
