...
\begin{document}
	...
	\begin{frame}{Joint Probability}
		\begin{itemize}
			\item When it is of interest to find the probability that two events will occur simultaneously, it is called joint probability
			\item For example, the probability that both event $A$ and $B$ will occur is given by: $P(A \cap B)$
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Law of Total Probability: Example}
		...
		Overall probability of a defective item is \(0.032\). \label{total-prob-example}
	\end{frame}


	\begin{frame}{Law of Total Probability (cont.)}
		For two events $E$ and $F$, the law can also be written as:
		\begin{align*}
	...
	\begin{frame}{Bayes' Theorem}
		Bayes' theorem reverses conditional probabilities. Assuming $P(B) > 0$:

		\begin{align}
			P(A \mid B) &= \frac{P(B \cap A)}{P(B)} \notag \\ 
						&= \frac{P(B \mid A)\,P(A)}{P(B)} \label{eq:bayes1}\\
						&= \frac{P(B \mid A)\,P(A)}{P(B \mid A)\,P(A) + P(B \mid A^c)\,P(A^c)} \label{eq:bayes2}
		\end{align}

		Equation (\ref{eq:bayes1}) ans (\ref{eq:bayes2}) are known as the Bayes' Rule.
		...
		% \textbf{Example:}
		% Let \(P(A) = 0.3\), \(P(B \mid A) = 0.5\), and \(P(B) = 0.4\).
		% \[
		% P(A \mid B) = \frac{0.5 \times 0.3}{0.4} = 0.375
		% \]
	...
	\begin{frame}{Bayes' Theorem (cont.)}
		...
		This theorem provies a framework to update one's belief when faced with new information.

		\vspace{0.5em}

		\begin{itemize}
			\item $P(A)$ is the \emph{prior} belief regarding the probability of event $A$
			\item The event $B$ has occured, is the new information
			\item $P(A \mid B)$ is the \emph{posterior} or the updated belief after it gets known that $B$ has occured
		\end{itemize}
	\end{frame}


	\begin{frame}{Bayes' Theorem: Example 1}
		Let \(P(A) = 0.3\), \(P(B \mid A) = 0.5\), and \(P(B) = 0.4\).
		\[
			P(A \mid B) = \frac{0.5 \times 0.3}{0.4} = 0.375
		\]
	...
	\begin{frame}{Bayes' Theorem: Example 2}
		Continuing from the example in page \ref{total-prob-example}, suppose an item is found to be defective, calculate the probability that:
		\begin{enumerate}
			\item it was produced by machine $B_1$
			\item it was produced by machine $B_2$
		\end{enumerate}
		
		\vspace{1em}

		Recall the following from page \ref{total-prob-example}:
	...
	\begin{frame}{Bayes' Theorem: Example 2 Solution}
		\begin{enumerate}
			\item Probability that the item was produced by $B_1$ given that the item was defective:
			\[
				P(B_1 \mid D) = \frac{P(D \mid B_1) \, P(B_1)}{P(D)} = \frac{0.02 \times 0.6}{0.032} = 0.375
			\]
			\item Probability that the item was produced by $B_2$ given that the item was defective:
			\[
				P(B_2 \mid D) = \frac{P(D \mid B_2) \, P(B_2)}{P(D)} = \frac{0.05 \times 0.4}{0.032} = 0.625
			\]
		\end{enumerate}
	\end{frame}


	\section{Calculating Approximate Probability from Contingency Table}
    ...
    \begin{frame}{Example: Contingency Table}
        ...
        \begin{tabular}{l|cc|c}
            ...
            \midrule
            Smoker                 & 55            & 35           & 90 \\
            Non-smoker             & 15            & 95           & 110 \\
            \midrule
            \textbf{Total}         & 70            & 130          & 200 \\
            \bottomrule
        \end{tabular}
        \end{center}
        \vspace{0.5em}
	...
	\begin{frame}{Probability from Contingency Table}
		Three types of probabilities can be calculated from contingency tables:
		\begin{itemize}
			\item \textbf{Marginal probability}: Marginal probability is the probability of a single event regardless of the other variable
			\item \textbf{Joint probability}: Joint probability is the probability that two events occur simultaneously
			\item \textbf{Conditional probability}: Conditional probability is the probability of an event occurring given that another event has already occurred
		\end{itemize}
	\end{frame}


	\begin{frame}{Marginal Probability}
		Marginal probability is the probability of a single event regardless of the other variable.

		It is calculated using the row totals or column totals.
		...
		\[
		P(\text{Smoker}) = \frac{90}{200} = 0.45
		\]

		Probability that a randomly selected individual has lung cancer:

		\[
		P(\text{Cancer}) = \frac{70}{200} = 0.35
		\]

		These probabilities come from the margins (totals) of the table.
	\end{frame}
	...
	\begin{frame}{Joint Probability}
		...
		In a contingency table, joint probabilities are obtained by dividing the cell frequency by the total number of observations.

		\textbf{Examples}:

		\[
		P(\text{Smoker and Cancer}) = \frac{55}{200} = 0.275
		\]

		\[
		P(\text{Non-smoker and No Cancer}) = \frac{95}{200} = 0.475
		\]

		Each cell in the table corresponds to a joint probability.
	\end{frame}
	...
	\begin{frame}{Conditional Probability}
		...
		Probability of lung cancer given that the person is a smoker:
		\[
		P(\text{Cancer}|\text{Smoker}) =
		\frac{55}{90} \approx 0.611
		\]

		Probability of lung cancer given that the person is a non-smoker:
		\[
		P(\text{Cancer}|\text{Non-smoker}) =
		\frac{15}{110} \approx 0.136
		\]

		Conditional probabilities use the row or column total corresponding to the given condition.
	\end{frame}
